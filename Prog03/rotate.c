...
void rotate90Left(ImageStruct* img1, ImageStruct* img2){
    ...
    for(int i=0; i<img1->height; i++){
        for(int j=0;j<img1->width;j++){
            raster2[j*img2->width+img2->width-i-1]=raster1[i*img1->width+j];
        }
    }
}
...
void rotate90Right(ImageStruct* img1, ImageStruct* img2){
    ...
    for(int i=0; i<img1->height; i++){
        for(int j=0;j<img1->width;j++){
            raster2[(img2->height-j-1)*img2->width+i]=raster1[i*img1->width+j];
        }
    }
}
...
void rotate180(ImageStruct* img1, ImageStruct* img2){
    ...
    for (unsigned int i=0; i<img1->height; i++) {
	  ...
	  raster2[(img2->height-i-1)*img2->width + (img2->width-j-1)] = raster1[i*img1->width + j];
        }
    }
}
