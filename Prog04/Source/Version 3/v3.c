...
This is the base file for version 3. It handles getting the file paths,
reading the content, then sending that content to different groups, which the
helper files (distributor and processor) handle in separate c programs
that are called by execvp().
...
/*----------------------------------------------------------------------------*\
This is the function that mainly changed from v2 to v3. It calls execvp()
so that it runs another c program with its own main. These processes run
the helper.c files and then terminate.
  ...
  for(int i=1; i<=procCount; i++){
    if(fork()==0){
			...
			for(int j=0; j<groupSize; j++) {
				fprintf(fp, "%s", group[j]);
			} fclose(fp);
			char buf1[10];
			char buf2[10];
			snprintf(buf1, 10, "%d", i);
			snprintf(buf2, 10, "%d", groupSize);
			char* args[] = {"./distributor", buf1, buf2, NULL};
			execvp(args[0], args);
    }
    usleep(10000);
  }
