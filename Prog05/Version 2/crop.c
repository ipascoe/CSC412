...
VERSION 2: ARGUMENTS ORIGINALLY PROVIDED ON COMMAND LINE ARE NOW PROVIDED IN STDIN FOR THE USE OF PIPES
----------------------------------------------------------------------------------------------------------------------------------*/
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
 ...
 */
int extractCropRegion(unsigned int imageWidth, unsigned int imageHeight,
...
//--------------------------------------------------------------
//	Main no longer takes arguments on the command line, instead
//	takes from stdin for the use of pipes.
//--------------------------------------------------------------
int main()
{
	int err=0;
	while(1) {
		...
		//	parameters of the crop region and number of threads
		unsigned int cropCornerX, cropCornerY, cropWidth, cropHeight, numThreads;

		fscanf(stdin, "%s %s %u %u %u %u %u", 
			inputImagePath, outFolderPath, &cropCornerX, &cropCornerY, 
			&cropWidth, &cropHeight, &numThreads);
		...
		extractCropRegion(imageIn.width, imageIn.height,
		...
		err = writeTGA(outFilePath, &imageOut);
