...
VERSION 2: ARGUMENTS WILL NOW BE PROVIDED IN STDIN FOR THE USE OF PIPES
--------------------------------------------------------------------------------------------------------------------*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
//--------------------------------------------------------------------------------------------
//  Only difference from version 1 is that the arguments for the functions are taken from
//  standard input so that they can read from the pipe in my script.
//--------------------------------------------------------------------------------------------
int main(){
    while(1) {

        //  Variables for the info being passes to stdin
        char inputPath[100];
        char outputPath[100];
        unsigned int numThreads;

        //  Scan stdin
        fscanf(stdin, "%s %s %u", inputPath, outputPath, &numThreads);
